...
const Dashboard: React.FC = () => {
    ...
    if (savedAddress) {
      axios
        ...
        .then((res) => {
          ...
          setDemoBalance(calculatedBalance);
        })
  ...
  const handleSimulatedTrade = (type: "buy" | "sell") => {
    ...
    if (savedAddress) {
      const payload = { ...newTrade, walletAddress: savedAddress };

      axios
        .post(`${API_BASE_URL}/api/simulated-trades`, payload)
        .then(res => {
          if (res.data && res.data.currentDemoBalance !== undefined) {
            // Use the authoritative balance from the server
            setDemoBalance(res.data.currentDemoBalance);
          } else {
            // Fallback to local calculation if server doesn't return balance
            setDemoBalance((prev) => (type === "buy" ? prev - amount : prev + amount));
          }
        })
        .catch((err) => {
          console.error("Failed to save trade", err);
          // Rollback the local trade if backend fails
          setSimulatedTrades((prev) => prev.filter(t => t.id !== newTrade.id));
        });
    } else {
      // If no saved address, just update locally
      setDemoBalance((prev) => (type === "buy" ? prev - amount : prev + amount));
    }
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative">
        ...
        <div className="mb-8 grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <motion.div
            className="bg-gray-800/50 backdrop-blur-sm p-6 rounded-lg border border-gray-700 cursor-pointer flex flex-col items-center justify-center text-center"
            whileHover={{ scale: 1.02 }}
            onClick={() => navigate('/user-activity')}
          >
            <Activity className="h-8 w-8 text-yellow-400 mb-2" />
            <h3 className="text-lg font-semibold mb-2">View Full Activity</h3>
            <p className="text-sm text-gray-400">
              See all your trades, activities, and detailed analytics
            </p>
            <motion.button
